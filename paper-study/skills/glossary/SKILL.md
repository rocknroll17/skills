...
**L1 — 한 줄 직관**
_중립적이고 간결한 한 문장 요약. 기술 용어 최소화. 정착된 영어 용어가 더 정확하면 원어를 그대로 쓴다. **비유는 구조적 동형이 있고 이해에 결정적일 때만** 한 문장. 장식성 비유는 금지하고, 직접 설명이 더 명료하면 그 쪽을 택한다._
...
_한 문단(3~5줄)으로 작동 방식을 풀이한다. 입력 → 내부 연산 → 출력 순서. 필요 시 짧은 ASCII 다이어그램._
...
L2·작동 원리에 쓸 정보가 논문 본문에 없으면 **WebSearch 한 번**으로 확보한다. 예:
...
확보한 배경은 `(논문 밖, WebSearch)` 표기와 출처 URL을 병기한다.
...
2. 없으면 위 포맷으로 새 엔트리. **L1은 직관** — 단순 정의 반복이 아니라 한 문장으로 핵심을 잡아준다.
...
- **연속 나열 금지**: 한 줄에 여러 용어를 뭉뚱그리지 않는다.
- **수학 기호 최소화**: L1은 기호 0개, L3에서만 필요한 만큼.
- **번역체 금지**: 어색한 직역 대신 자연스러운 한국어. 정착된 영어 용어는 원어 그대로.
