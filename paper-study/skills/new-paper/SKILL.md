...
3. **역할 부여**: 각 구획에 아래 역할 프로필 중 하나를 배정한다. 역할이 본문 구조를 결정한다 — 논문 섹션은 기능이 서로 다르므로 note 구조도 달라야 한다.
4. **파일명 부여**: `notes/NN-<영문-kebab-slug>.md`, `00`은 overview 예약. 번호는 **추천 읽기 순서** (논문 순서와 달라도 된다).
5. **파일 생성**: 각 구획마다 `notes/_section-template.md`를 복사해 `{{SECTION_TITLE}}`·`{{ROLE}}`을 치환한다.
6. 설계한 구획 목록(파일명 + 역할 + 담당 범위 §X~§Y)을 사용자에게 한 줄씩 보고하고 진행한다.

### 구획 역할 프로필 (본문 구조가 역할마다 다르다)

같은 틀을 모든 구획에 반복하면 읽는 쪽이 지친다. 역할별로 본문 골격을 달리한다:

| 역할 | 본문 골격 | 분량 |
| --- | --- | --- |
| **문제·동기** | 현재 상황(합의된 지형) → 균열(무엇이 안 되나, 구체 시나리오) → 이 논문의 응답 1문장 | 100~200줄 |
| **맥락·계보** | 선행 흐름을 2~4갈래로 지도화 → 갈래별 대표작과 한계 → 본 논문이 서는 자리 | 100~180줄 |
| **핵심 기여** | 5블록 인과 체인: What-problem → Why-hard → Key-idea → How-works → So-what | 200~350줄 |
| **근거·검증** | 논문의 주장 목록 → 검증 설계(무엇을 재나) → 결과 → **주장별 판정** (지지/부분/미지지) | 150~300줄 |
| **한계·전망** | 깨지는 조건 → 저자가 인정한 한계 vs 안 한 한계 → 열린 질문 | 60~120줄 |

역할이 애매하면 가장 가까운 것을 고르고 골격을 변형한다 (예: 이론 논문의 증명 구획 = 핵심 기여 골격에서 How-works를 증명 아이디어로).

### 단조로움 방지 (구획 간 변주)

- **도입 변주**: 각 note의 첫 문장은 직전 구획과 다른 방식으로 연다 — 구체 시나리오 / 반직관적 사실 / 독자에게 던지는 질문 중 택1.
- **Self-check 변주**: 회상("한 문장으로?") → 적용("이 조건이 바뀌면?") → 비판("저자 주장의 약한 고리는?")을 구획마다 순환.
- 공통 유지: stuck-points·failure-case·self-check 3종 부착물은 모든 구획에 필수.
...
#### 단계 4 — 구획 본문 작성 (역할 골격)
구획에 배정된 **역할 프로필의 골격**을 따라, 사람이 이해하는 순서로 재구성한다 (논문 서술 순서와 무관). 문체는 `CLAUDE.md §3·§5`를 준수한다. 역할 무관 공통:

- 말하듯 쓴 산문 2~4문단, 인과 접속을 적극 활용. 수식 등장 시 5블록 축약 해설.
- 복잡한 구조는 ASCII 다이어그램. 비유는 구조적 동형이 있을 때만.
- 마무리에 다른 구획 cross-reference 1회 이상.
- 도입은 "단조로움 방지" 규칙대로 직전 구획과 다르게 연다.
...
- notes/NN-<구획>.md          ✅ (설계한 구획 수만큼 나열, 역할·담당 범위 §X~§Y 병기)
